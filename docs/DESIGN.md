...
| # | 禁止 | 理由 |
|---|------|------|
| 1 | `font-size` に 14px 以下を使う | 60代が読めない |
...
- アイコンサイズ: 24px。ラベルは12pxではなく **13px**（最小15px例外: タブラベルのみ許容）
...
- sub: `font-size: 13px`, trend=up → `color: var(--color-checkin)`, trend=down → `color: var(--color-error)`
...
8. **料金プレビュー**（自動計算。リアルタイム更新）:
   - 宿泊料 × 人数 × 泊数
   - 食事料 × 人数 × 泊数
   - **宿泊税**（★必須。settingsの税率設定から計算。現行バグ: ¥0になっている）
   - 合計
