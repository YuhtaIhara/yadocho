...
const schema = z
  .object({
    ...
    breakfast: z.boolean(),
    lunch: z.boolean(),
    dinner_count: z.number().min(0).max(50),
    breakfast_count: z.number().min(0).max(50),
    lunch_count: z.number().min(0).max(50),
...
export default function ReservationForm({ mode = 'create', initialData }: Props) {
  ...
  async function onSubmit(data: FormValues) {
    ...
    if (validRoomIds.length === 0) {
      setFormError('選択した部屋はすべて予約済みまたは休業日です。別の部屋を選んでください。')
      return
    }
