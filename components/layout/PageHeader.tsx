      ...
      <div className="flex items-center h-14 px-5 gap-3">
        {showBack && (
          <button
            type="button"
            onClick={() => router.back()}
            className="w-12 h-12 min-w-[48px] min-h-[48px] flex items-center justify-center rounded-full active:bg-primary-soft transition-colors"
          >
            <ArrowLeft size={20} className="text-text-2" />
          </button>
        )}
        <h1 className="text-xl font-medium flex-1 min-w-0 truncate">{title}</h1>
